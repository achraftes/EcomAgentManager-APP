<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                ...
                @if (Auth::user()->role !== 'agent')
                    <x-nav-link :href="route('mediaBuyers.index')" :active="request()->routeIs('mediaBuyers.index')">
                        {{ __('Media Buyer') }}
                    </x-nav-link>
                @endif
                ...
                @if (Auth::user()->role !== 'agent')
                    ...
                    @endif
                    <x-nav-link :href="route('agents.stats', ['agentId' => Auth::user()->id])" :active="request()->routeIs('agents.stats')">
                        {{ __('Agent Stats') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-400 bg-gray-800 hover:text-gray-300 hover:bg-gray-900 focus:outline-none transition ease-in-out duration-150">
                            <div>
                                @if (Auth::check())
                                    {{ Auth::user()->name }}
                                @else
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->role !== 'agent')
                <x-responsive-nav-link :href="route('mediaBuyers.index')" :active="request()->routeIs('mediaBuyers.index')">
                    {{ __('Media Buyer') }}
                </x-responsive-nav-link>
            @endif
            ...
            @if (Auth::user()->role !== 'agent')
                ...
                @endif
                <x-responsive-nav-link :href="route('agents.stats', ['agentId' => Auth::user()->id])" :active="request()->routeIs('agents.stats')">
                    {{ __('Agent Stats') }}
                </x-responsive-nav-link>
